    ...
    public class UnaryArithmeticExpressionNodeTests
    {
        ...
        public void UnaryNegativeInteger()
        {
            ...
            ITestableObserver<int> results = scheduler.Start(
                () => result(input.AsQbservable()).Select(x => int.Parse(x.GetType().GetProperty("resultado").GetValue(x).ToString())),
                created: 10,
                subscribed: 50,
                disposed: 400);
        ...
        public void UnaryNegativeDouble()
        {
            ...
            ITestableObserver<double> results = scheduler.Start(
                () => result(input.AsQbservable()).Select(x => double.Parse(x.GetType().GetProperty("resultado").GetValue(x).ToString())),
                created: 10,
                subscribed: 50,
                disposed: 400);
        ...
        public void UnaryNegativeDecimal()
        {
            ...
            ITestableObserver<decimal> results = scheduler.Start(
                () => result(input.AsQbservable()).Select(x => decimal.Parse(x.GetType().GetProperty("resultado").GetValue(x).ToString())),
                created: 10,
                subscribed: 50,
                disposed: 400);
